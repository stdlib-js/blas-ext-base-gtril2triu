...
*
* @param {(integer|string)} order - storage layout of `A` and `B`
...
* // B => [ 0.0, 3.0, 0.0, 0.0 ]
*/
function gtril2triu( order, M, N, k, A, LDA, B, LDB ) {
	var isrm;
	var sa1;
	var sa2;
	var sb1;
	...
	if ( l === null ) {
		throw new TypeError( format( '36jFx', order ) );
	}
	isrm = isRowMajor( l );
	if ( isrm ) {
	...
	if ( LDA < max( 1, sa ) ) {
		throw new RangeError( format( '36jJL', sa, LDA ) );
	}
	if ( LDB < max( 1, sb ) ) {
		throw new RangeError( format( '36jGM', sb, LDB ) );
	}
	if ( isrm ) {
		sa1 = LDA;
		sa2 = 1;
